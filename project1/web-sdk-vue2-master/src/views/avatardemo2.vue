    ...
    <div class="avatar">
        <el-container>
            <el-aside width="200px" height="840px">
                <el-row>
                    <el-button type="primary" @click="switchAvatar">切换到苏轼</el-button>
                    <el-button style="margin: 0px" @click="initSDK()" type="primary">实例化SDK</el-button>
                    <el-button style="margin: 0px" @click="createRecoder()" type="primary">创建录音器</el-button>
                    <el-button style="margin: 0px" @click="setSDKEvenet()" type="primary">设置SDK监听事件</el-button>
                    <el-button style="margin: 0px" @click="setPlayerEvenet()" type="primary">设置播放器监听事件</el-button>
                    <el-button style="margin: 0px" @click="SetApiInfodialog = true"
                        type="primary">SetApiInfo</el-button>
                    <el-button style="margin: 0px" @click="SetGlobalParamsdialog = true"
                        type="primary">SetGlobalParams</el-button>
                    ...
                    <el-input type="textarea" placeholder="请输入内容,包括符号在内，最大2000字符" v-model="textarea" maxlength="2000"
                        show-word-limit :autosize="{ minRows: 5, maxRows: 10 }">
                    </el-input>
                    ...
                    <el-button style="margin: 0px" @click="startRecord()" v-if="recorderbutton == false"
                        type="primary">开启录音</el-button>
                    <el-button style="margin: 0px" @click="stopRecord()" v-if="recorderbutton == true"
                        type="primary">关闭录音</el-button>
            ...
            <el-dialog title="设置全局变量" :visible.sync="SetGlobalParamsdialog">
                ...
                <el-form :model="setglobalparamsform" :label-width="formLabelWidth">
                    <el-form-item label="视频协议">
                        <el-tooltip class="item" effect="dark" content="支持webrtc/xrtc/rtmp(控制台打印视频流地址)"
                    ...
                    <el-form-item label="分辨率高">
                        <el-input class="widthclass" v-model="setglobalparamsform.avatar.height"
                            autocomplete="on"></el-input>
                    </el-form-item>
                    <el-form-item label="分辨率宽">
                        <el-input class="widthclass" v-model="setglobalparamsform.avatar.width"
                            autocomplete="on"></el-input>
                    </el-form-item>
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" @click="(SetGlobalParamsdialog = false), SetGlobalParams()">确
                        定</el-button>
                </div>
            </el-dialog>
        </el-container>
...
export default {
    ...
    data() {
        return {
            ...
            form: {
                appid: "",//启动前请填入讯飞开放平台凭证（接口服务中获取，请勿提交真实凭证）
                apikey: "",//启动前请填入讯飞开放平台凭证（接口服务中获取，请勿提交真实凭证）
                apisecret: "",//启动前请填入讯飞开放平台凭证（接口服务中获取，请勿提交真实凭证）
                sceneid: "",//启动前请填入讯飞开放平台"接口服务ID"（到交互平台-接口服务中获取，请勿提交真实值）
            ...
            setglobalparamsform: {
                ...
                avatar: {
                    avatar_id: "111140001",//（必传）授权的形象资源id，请到交互平台-接口服务-形象列表中获取
                    width: 1000,//（非必传）视频分辨率宽（不是画布的宽，调整画布大小需调整名为wrapper的div宽）
                    height: 1920,//（非必传）视频分辨率高（不是画布的高，调整画布大小需调整名为wrapper的div高）
                    mask_region: "[200,0,1080,1920]",//（非必传）形象裁剪参数，[从左到右，从上到下，从右到左，从下到上]
                    scale: 1,//（非必传）形象缩放比例，取值范围0.1-1
                    move_h: 30,//（非必传）形象左右移动
                    move_v: 0,//（非必传）形象上下移动
                    audio_format: 1,//（非必传）音频采样率，传1即可
                },
                tts: {
                    vcn: "x4_chaoge",//（必传）授权的声音资源id，请到交互平台-接口服务-声音列表中获取
                    speed: 50,//（非必传）语速
                    pitch: 50,//（非必传）语调
                    volume: 100,//（非必传）音量
